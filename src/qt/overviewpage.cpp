...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeYuanCoinAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeYuanCoinAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeYuanCoinAmount = strAnonymizeYuanCoinAmount.remove(strAnonymizeYuanCoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeYuanCoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeYuanCoinAmount*COIN) nMaxToAnonymize = nAnonymizeYuanCoinAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeYuanCoinAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeYuanCoinAmount));
        strAnonymizeYuanCoinAmount = strAnonymizeYuanCoinAmount.remove(strAnonymizeYuanCoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeYuanCoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
